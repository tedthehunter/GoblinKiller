...
func update_visibility(player_position: Vector2) -> void:
	...
	for cell in level.get_walkable_cells(MapDiscovery.CELL_SIZE):
		...
		if player_position.distance_to(center) <= VISIBILITY_RADIUS and level.has_line_of_sight(player_position, center):
			discovery.reveal(center)
	queue_redraw()
...
func _draw() -> void:
	...
	for cell in level.get_walkable_cells(MapDiscovery.CELL_SIZE):
		...
		var alpha := 0.62 if discovery.is_discovered(center) else 0.94
